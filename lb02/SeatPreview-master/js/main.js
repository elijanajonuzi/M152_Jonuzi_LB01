...
;(function(window) {
	...
	var support = {transitions : Modernizr.csstransitions},
		...
		tiltRotation = { //x=vertically y=horizontally
			rotateX : 50, // a relative rotation of -25deg to 25deg on the x-axis
			rotateY : 20  // a relative rotation of -15deg to 15deg on the y-axis
		},
		// controls whether the tilt is active or not
		tilt = false, // is 180° button as default activated
		// window sizes
		...
		transitionOpts = {'speed' : 1000, 'easing' : 'cubic-bezier(.7,0,.3,1)'}, //https://cubic-bezier.com/#.2,.51,1,1  -- 1.=%of time -- 2.=%of progress of movement at 1. -- 3.=%of time at end -- 4.=%of progress at 2.

		// the room dimentions
		roomsize = { //don't mess with the room size
  ...
  //applies change of camera position
  ...
  //time and other settings of change of camera position
  ...
  //defines room scales at beginning
	function scaleRoom() {
	...
	function selectSeat(planseat) {
		if( classie.has(planseat, 'row__seat--reserved') ) {//returns false because the seat is already reserved
			return false;
		}
		if( classie.has(planseat, 'row__seat--selected') ) { //remove seat from selection on plan
			classie.remove(planseat, 'row__seat--selected');
			return false;
		}
		// add selected class
		classie.add(planseat, 'row__seat--selected'); //selects seat
